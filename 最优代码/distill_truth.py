...
def main():
    ...
    args = ap.parse_args()
    # 正式提交只开放最终权重实际采用的 14 个温度通道真值续训路径。
    if (args.dtp or args.phys or args.disk_cache or args.chanrw or args.regionw or
            args.truth_windq or args.truth_full15 or args.hfspec or args.hf_ms):
        ap.error("正式复现配置固定为 dtp=0, phys=0, disk-cache='', chan/region reweight=0, truth-windq=0, truth-full15=0, hf-spec=0")
    global T_UPPER_IDX
    if args.full_temp:
    ...
    if resume_from:
        ...
        miss = student.load_state_dict(rsd, strict=True)
    ...
    for ep in range(start_ep, args.epochs):
        ...
        # 选优口径:val_match + 温度对真值RMSE(越小越好)→ 用复合分(真值监督核心目标是温度)
        sel = vrel - 0.0  # 主用 val_match 保整体;温度单独打印观察
        if sel > best:
            best = sel
